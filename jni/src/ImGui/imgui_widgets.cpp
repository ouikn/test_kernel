...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    // 计算滑块框架的宽度
    float slider_width = window->SizeFull.x * 0.23f;  //滑块宽度占窗口宽度的25%
    ...
    const bool hovered = ItemHoverable(new_frame_bb, id, g.LastItemData.ItemFlags); 
    bool temp_input_is_active = temp_input_allowed && TempInputIsActive(id);
    if (!temp_input_is_active)
    {
    ...
    // 使用提取的坐标来绘制框架
    RenderFrame(
        ImVec2(new_frame_bb.Min.x, new_frame_bb.Min.y + 23.5f), 
        ImVec2(new_frame_bb.Max.x, new_frame_bb.Max.y - 23.5f), 
        frame_col,
        true,
    ...
    window->DrawList->AddRectFilled(
        ImVec2(new_frame_bb.Min.x, new_frame_bb.Min.y + 23.5f), 
        ImVec2(fill_max_x, new_frame_bb.Max.y - 23.5f), 
        ImColor(50, 150, 255, 255),
        g.Style.GrabRounding
    );
    ...
    window->DrawList->AddText(NULL, g.FontSize, text_pos, GetColorU32(ImGuiCol_Text), value_buf, value_buf_end);
